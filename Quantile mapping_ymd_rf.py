...
"""
Code Flowchart:
Rainfall Analysis GUI Flowchart
1. Start Application
    Initialize RainfallAnalysisGUI and enter mainloop.

2. Input & Setup
    Initialize GUI (setup_gui)
        Configure data input, results, and plots tabs.
    Set Parameters
        Set defaults: storm threshold, quartile points, spline method.



3. Data Preparation
Load Data
    Satellite Data (load_sat_data)
        Select Excel/CSV file.
        Choose time and precipitation columns (select_columns).
        Rename to Date, Satellite.
        
    round Data (load_ground_data)
        Select Excel/CSV file.
        Choose time and precipitation columns (select_columns).
        Rename to Date, Ground.

    Aggregate Data (aggregate_data)
        Compute daily, monthly, or yearly aggregates (mean, max, total).


    Validate Data (validate_data)
        Check for duplicates, datetime format, NaNs.
        Find common dates between satellite and ground.
        Decision: Common Dates ≥ 10?
            Yes: Proceed to merge.
            No: Error: Insufficient common dates → Stop.
        Merge datasets on common dates.
        Log stats: mean, std, zeros, NaNs.
        Decision: Non-Zero Data ≥ 5?
            Yes: Proceed to preprocessing.
            No: Error: Insufficient non-zero data → Stop.

    Preprocess Data
        Decision: Zero-Rain Adjustment? (Daily data only)
            Yes: Adjust zero-rain (adjust_zero_rain)
                Match ground zero fraction.
            No: Skip adjustment.
        Set values < 1 mm to 0 (daily data).
        Decision: Outlier Capping?
            Yes: Cap at 98th percentile.
            No: Skip capping.


    Analyze Storm Offsets (analyze_storm_offsets)
        Identify storms (threshold: manual or 75th percentile).
        Compute offset histogram and weighted median.
        Decision: Median Offset ≠ 0 & ≤ 3?
            Yes: Shift satellite data.
            No: No shift.

4. Analysis & Evaluation
    Apply Bias Correction
        Decision: Full Satellite Data?
            Yes: Use full satellite data, apply zero-rain and outlier adjustments.
            No: Use common dates dataset.

    Linear Quantile Mapping (linear_quantile_mapping)
        Interpolate between quartile points.


    Rank-Based Mapping (rank_based_mapping)
        Predict rain events, map non-zero ranks.
        Decision: Enough Non-Zero Data?
            Yes: Map ranks.
            No: Fallback to linear mapping.

    Spline Quantile Mapping (spline_quantile_mapping)
        Decision: Original Spline?
            Yes: Adaptive knots (spline_quantile_mapping_original).
            No: Simplified spline (spline_quantile_mapping_simplified).

    Evaluate Metrics (calculate_metrics)
        Compute RMSE, MAE, Bias, POD, FAR, KGE, KS.

    Predict Dry Days (predict_rain)
        Evaluate dry day agreement.

    Evaluate Correction Methods (evaluate_correction_methods)
        Compare zero fractions, KS statistics.
...
5. Output
    ...
    Generate Plots (generate_plots)
        Create histogram (ground, original, linear, rank, spline).
        Create CDF (empirical distributions).
        Create time series (user-defined range or first 100 days).
        Create metrics comparison (bar plot: RMSE, MAE, etc.).

    Save Outputs
        Save data, metrics, parameters to Excel.
        Save plots as PNG.
        Save log file.
